...
export function verseUrl(ref: VerseRef): string {
  const m = /^(\S+) (.+)$/.exec(ref);
  const book = m ? BOOK_SLUGS[m[1]!] ?? m[1]! : ref;
  return `https://www.biblegateway.com/passage/?search=${encodeURIComponent(`${book} ${m?.[2] ?? ""}`)}`;
}

/** Tappable verse citation chips linking to a Bible reader. */
export function VerseChips({ refs }: { refs: VerseRef[] }): React.ReactElement {
  return (
